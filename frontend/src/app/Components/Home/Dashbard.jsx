...
import Background from "../Home/Image/background.jpeg"
...
export default function Dashboard({pdfRef,downloadPDF}) {
  ...
  //         pdf.addImage(img, "PNG", imgX, imgY, img.width * ratio, img.height * ratio);
  ...
  const [items, setItems] = useState([
    {
      ...
      qty: 1,
      rate: 0,
      subitems: [{ subitemname: "seal replacement", qty: 1, rate: 500 }],
    },
  ]);
  ...
  const totalAmount = items.reduce((sum, item) => {
    const subSum = item.subitems?.reduce(
      (s, sub) => s + sub.qty * sub.rate,
      0
    );
  ...
  async function AddData() {
    try {
      ...
      const response = await axios.post("http://localhost:8000/balaji/post", body);
   ...
   <div ref={pdfRef}>
     <div  className="relative bg-amber-50 text-black min-h-screen p-4 space-y-6">
      <img
        src={Background.src}
        alt="Watermark"
        className="absolute opacity-100 w-[500px] top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none z-0"
      />
...
<div className="flex justify-between gap-6">
  <div className="w-1/5">
    ...
    <input
      ...
      onChange={(e)=>SetCode((prev)=>({...prev,InvoiceNo:e.target.value}))}
    />
  </div>
  ...
  <div className="w-1/5 text-right">
    ...
    <input
      ...
      onChange={(e)=>SetCode((prev)=>({...prev,state:e.target.value}))}
    />
  </div>
</div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="space-y-2">
            ...
            <input
              className="w-full border border-amber-300 px-2 py-1 rounded"
        ...
        <div className="text-sm space-y-1">
          <p><strong>Bank:</strong> Axis Bank, Narela Delhi-110040</p>
          <p><strong>Account No:</strong> 919020012035899</p>
          <p><strong>IFSC:</strong> UTIB0001260</p>
        </div>

        <div className="text-right">
          <p className="italic">For Bala Ji Pump Repairing Centre</p>
